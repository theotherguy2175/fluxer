...
<sup>1</sup> Skipped when no messages are collected
...
<sup>1</sup> The `selected` scope requires at least one of the include fields to be true, and a body disabling them all is rejected at path `include_dms`
...
Fluxer prepares the archive in the background. It contains:
...
The `token` query parameter issued by [Get data harvest download URL](#get-data-harvest-download-url) authorises the download without a session. Use the complete issued URL and keep it secret.

...
title: Admin voice
description: Manage voice regions, servers, credentials, and placement eligibility.
---
...
A 500 response can arrive after the configuration has changed. Read it back before retrying.
:::
...
The operator supplies `id` on creation. Channels reference it through `rtc_region`, and [Modify call region](/http-api/calls/#modify-call-region) accepts it. To change an ID, create a new region and delete the old one.
...
| allowed_guild_ids<sup>2</sup><sup>3</sup> | array[snowflake] | The guilds admitted without checking the other guild gates (max 1000) |
...
| allowed_guild_ids<sup>4</sup> | array[snowflake] | The guilds admitted without checking the other guild gates (max 1000) |
...
<RouteHeader method="GET" path="/v1/admin/voice/regions" auditReason />
...
The operation records one [Admin audit entry](/admin-api/#admin-audit-entry-object) with action `list_voice_regions`, target type `voice_region`, target ID `0`, and metadata keys `include_servers` and `result_count`.
...
<RouteHeader method="GET" path="/v1/admin/voice/regions/{region_id}" auditReason />
...
The operation records one [Admin audit entry](/admin-api/#admin-audit-entry-object) with action `get_voice_region`, target type `voice_region`, target ID `0`, and metadata keys `region_id`, `include_servers`, and `found`. An unknown region records `found` as `false`.
...
| allowed_guild_ids?<sup>1</sup> | array[snowflake] | The guilds admitted without checking the other guild gates (max 1000) |
...
There is no confirmation step or recovery operation. Save the server configuration and credentials separately before deleting a region if they will be needed again. Server reads do not return credentials.
:::
...
<RouteHeader method="GET" path="/v1/admin/voice/regions/{region_id}/servers" auditReason />
...
The operation records one [Admin audit entry](/admin-api/#admin-audit-entry-object) with action `list_voice_servers`, target type `voice_server`, target ID `0`, and metadata keys `region_id` and `result_count`.
...
<RouteHeader method="GET" path="/v1/admin/voice/regions/{region_id}/servers/{server_id}" auditReason />
...
The operation records one [Admin audit entry](/admin-api/#admin-audit-entry-object) with action `get_voice_server`, target type `voice_server`, target ID `0`, and metadata keys `region_id`, `server_id`, and `found`. A pair that resolves to no server records `found` as `false`.
...
| allowed_guild_ids?<sup>3</sup> | array[snowflake] | The guilds admitted without checking the other guild gates (max 1000) |

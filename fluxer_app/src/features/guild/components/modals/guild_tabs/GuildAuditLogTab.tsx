...
import {
	...
	getLegacyTranslatedAuditLogActions,
	getTranslatedAuditLogActions,
	LEGACY_AUDIT_LOG_ACTIONS,
...
import GuildActivityLogPresentationRollout from '@app/features/guild/state/GuildActivityLogPresentationRollout';
...
const getLegacyActionIcon = (actionType: AuditLogActionType): IconComponent => {
	const targetType = getTargetType(actionType);
	return targetIconMap[targetType as AuditLogTargetType] ?? actionIconMap[actionType] ?? BuildingsIcon;
};
const getLegacyActionOptionIcon = (value: string): IconComponent => {
	if (!value) return FunnelSimpleIcon;
	const action = LEGACY_AUDIT_LOG_ACTIONS.find((item) => item.value.toString() === value);
	if (!action) return FunnelSimpleIcon;
	const actionType = Number(value) as AuditLogActionType;
	const targetType = getTargetType(actionType);
	return targetIconMap[targetType as AuditLogTargetType] ?? getLegacyActionIcon(actionType);
};
...
const LegacyGuildAuditLogTab: React.FC<{guildId: string}> = observer(({guildId}) => {
	const {i18n} = useLingui();
	const [entries, setEntries] = useState<Array<GuildAuditLogEntryResponse>>([]);
	const [isLoading, setIsLoading] = useState(false);
	const [error, setError] = useState<string | null>(null);
	...
	const actionOptions = useMemo<Array<ComboboxOption<string>>>(
		() => [
			{value: '', label: i18n._(ALL_ACTIONS_DESCRIPTOR)},
			...getLegacyTranslatedAuditLogActions(i18n).map((action) => ({
				value: action.value.toString(),
				label: action.label,
			})),
		],
		...
		const renderContent = (option: UserFilterOption) => {
			if (!option.value) {
				return (
					<div
						className={styles.userSelectRowGlobal}
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.user-select-row-global"
					>
						<span
							className={styles.userSelectLabel}
							data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.user-select-label"
						>
							{option.label}
						</span>
					</div>
			...
			return (
				<div
					className={styles.userSelectRow}
					data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.user-select-row"
				>
					{isAuditLogUserOption(option) && (
						<div
							className={styles.userSelectAvatarWrapper}
							data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.user-select-avatar-wrapper"
						>
							<Avatar
								user={option.user}
								size={USER_FILTER_AVATAR_SIZE}
								guildId={guildId}
								data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.avatar"
							/>
						</div>
					)}
					<span
						className={styles.userSelectLabel}
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.user-select-label--2"
					>
						{option.label}
					</span>
				</div>
	...
	const actionComboboxRenderers = useMemo(() => {
		const renderContent = (option: ComboboxOption<string>) => {
			const Icon = getLegacyActionOptionIcon(option.value);
			const actionKind = option.value ? getActionKind(Number(option.value) as AuditLogActionType) : null;
			const actionToneClass =
				actionKind != null ? getActionSelectIconToneClass(actionKind) : styles.actionSelectIconNeutral;
			return (
				<div
					className={styles.actionSelectRow}
					data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.action-select-row"
				>
					<span
						className={clsx(styles.actionSelectIcon, actionToneClass)}
						aria-hidden
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.action-select-icon"
					>
						<Icon
							size={remFromPx(18)}
							weight="bold"
							data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.icon"
						/>
					</span>
					<span
						className={styles.actionSelectLabel}
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.render-content.action-select-label"
					>
						{option.label}
					</span>
				</div>
	...
	return (
		<div
			...
			data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.container"
		>
			<div
				className={styles.headerTop}
				data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.header-top"
			>
				<h2
					className={styles.pageTitle}
					data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.page-title"
				>
					{i18n._(ACTIVITY_LOG_DESCRIPTOR)}
				</h2>
				<p
					className={styles.pageSubtitle}
					data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.page-subtitle"
				>
					{i18n._(TRACK_MODERATOR_ACTIONS_ACROSS_THE_COMMUNITY_DESCRIPTOR)}
				</p>
			</div>
			<div
				...
				data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.filter-row"
			>
				<Combobox
					value={selectedUserId}
					onChange={(value) => setSelectedUserId(value)}
					...
					renderValue={userComboboxRenderers.renderValue}
					label={i18n._(FILTER_BY_USER_DESCRIPTOR)}
					data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.select"
				/>
				<Combobox
					value={selectedAction}
					onChange={(value) => setSelectedAction(value)}
					...
					renderOption={actionComboboxRenderers.renderOption}
					renderValue={actionComboboxRenderers.renderValue}
					data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.select--2"
				/>
			</div>
			<div
				className={styles.entries}
				data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.entries"
			>
				{isLoading && entries.length === 0 && (
					<div
						className={styles.spinnerRow}
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.spinner-row"
					>
						<Spinner size="large" data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.spinner" />
					</div>
				)}
				{entries.length > 0 && !shouldShowErrorState && (
					<div
						className={styles.entryList}
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.entry-list"
					>
						{entries.map((entry) => {
							const entryId = entry.id;
							const targetType = getTargetType(entry.action_type as AuditLogActionType);
							...
							const actionClassKey = `type_${actionKind}` as keyof typeof styles;
							const entryClasses = clsx(styles.auditLog, styles[targetClassKey], styles[actionClassKey]);
							const ActionIcon = getLegacyActionIcon(entry.action_type as AuditLogActionType);
							const actorUser = entry.user_id ? (Users.getUser(entry.user_id) ?? null) : null;
								...
								.map((change, changeIndex) => {
									...
									return (
										<div
											...
											data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-item"
										>
											<span
												className={clsx(styles.changeBullet, toneClass)}
												aria-hidden
												data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-bullet"
											>
												<ChangeIcon
													size={remFromPx(12)}
													weight="bold"
													className={styles.changeBulletIcon}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-bullet-icon"
												/>
											</span>
											<span
												className={styles.changeText}
												data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-text"
											>
							...
							return (
								<div
									...
									data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.div"
								>
									{isExpandable ? (
										<FocusRing
											offset={-2}
											data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.focus-ring"
										>
											<button
												type="button"
												onClick={() => toggleExpanded(entryId)}
												className={headerClasses}
												aria-expanded={isExpandedView}
												data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.button.toggle-expanded"
											>
												<span
													className={styles.icon}
													aria-hidden
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.icon"
												>
													<ActionIcon
														size={remFromPx(20)}
														weight="bold"
														className={styles.iconGlyph}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.icon-glyph"
													/>
												</span>
												<div
													className={styles.avatar}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.avatar"
												>
													{actorUser ? (
														<Avatar
															user={actorUser}
															size={32}
															guildId={guildId}
															data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.avatar--2"
														/>
													) : (
														<MockAvatar
															size={32}
															userTag={entry.user_id ?? i18n._(UNKNOWN_USER_DESCRIPTOR)}
															data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.mock-avatar"
														/>
													)}
												</div>
												<div
													className={styles.textBlock}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.text-block"
												>
													<div
														className={styles.titleRow}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.title-row"
													>
														<span
															className={styles.summary}
															data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.summary"
														>
															{summaryNode}
														</span>
													</div>
													<div
														...
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.meta-row"
													>
														<span
															className={styles.timestamp}
															data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.timestamp"
														>
															{formatTimestamp(entry.id)}
														</span>
													</div>
												...
												<CaretDownIcon
													...
													weight="bold"
													className={clsx(styles.chevron, {[styles.chevronExpanded]: isExpandedView})}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.chevron"
												/>
											</button>
										</FocusRing>
									) : (
										<div
											className={headerClasses}
											data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.div--2"
										>
											<span
												className={styles.icon}
												aria-hidden
												data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.icon--2"
											>
												<ActionIcon
													size={remFromPx(20)}
													weight="bold"
													className={styles.iconGlyph}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.icon-glyph--2"
												/>
											</span>
											<div
												className={styles.avatar}
												data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.avatar--3"
											>
												{actorUser ? (
													<Avatar
														user={actorUser}
														size={32}
														guildId={guildId}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.avatar--4"
													/>
												) : (
													<MockAvatar
														size={32}
														userTag={entry.user_id ?? i18n._(UNKNOWN_USER_DESCRIPTOR)}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.mock-avatar--2"
													/>
												)}
											</div>
											<div
												className={styles.textBlock}
												data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.text-block--2"
											>
												<div
													className={styles.titleRow}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.title-row--2"
												>
													<span
														className={styles.summary}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.summary--2"
													>
														{summaryNode}
													</span>
												</div>
												<div
													className={styles.metaRow}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.meta-row--2"
												>
													<span
														className={styles.timestamp}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.timestamp--2"
													>
														{formatTimestamp(entry.id)}
													</span>
												</div>
									...
									{isExpandedView && (
										<div
											...
											data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.details"
										>
											{shouldShowReasonPreview && (
												<div
													className={styles.reasonRow}
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.reason-row"
												>
													<span
														className={styles.reasonLabel}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.reason-label"
													>
														<Trans>Reason</Trans>
													</span>
													<span
														className={styles.reasonValue}
														data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.reason-value"
													>
														{reasonText}
													</span>
												</div>
											...
											{(changeRows.length > 0 || optionEntries.length > 0) && (
												<div
													...
													data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-list"
												>
													{changeRows}
													{optionEntries.map(([key, value]) => (
														<div
															className={styles.changeItem}
															key={key}
															data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-item--2"
														>
															{(() => {
																...
																return (
																	<span
																		className={clsx(styles.changeBullet, toneClass)}
																		aria-hidden
																		data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-bullet--2"
																	>
																		<ChangeIcon
																			size={remFromPx(12)}
																			weight="bold"
																			className={styles.changeBulletIcon}
																			data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-bullet-icon--2"
																		/>
																	</span>
																);
															})()}
															<span
																className={styles.changeText}
																data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.change-text--2"
															>
																{renderOptionDetailSentence(
																	key,
																	value,
				...
				{shouldShowEmptyState && (
					<div
						...
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.empty-state"
					>
						<EmptySlate
							Icon={ClipboardTextIcon}
							title={i18n._(NO_LOGS_YET_DESCRIPTOR)}
							description={i18n._(MODERATION_ACTIONS_AND_COMMUNITY_CHANGES_WILL_APPEAR_HERE_DESCRIPTOR)}
							data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.empty-slate"
						/>
					</div>
				)}
				{!isLoading && shouldShowErrorState && (
					<div
						...
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.error-state"
					>
						<EmptySlate
							Icon={WarningCircleIcon}
							title={i18n._(UNABLE_TO_LOAD_ACTIVITY_LOGS_DESCRIPTOR)}
							description={errorDescription}
							data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.empty-slate--2"
						/>
						<div
							className={styles.statusActions}
							data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.status-actions"
						>
							<Button
								variant="secondary"
								onClick={() => loadLogs({reset: true})}
								data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.button.load-logs"
							>
								{i18n._(TRY_AGAIN_DESCRIPTOR)}
							</Button>
						</div>
				...
				{hasMore && entries.length > 0 && !shouldShowErrorState && (
					<div
						...
						data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.load-more"
					>
						<div
							ref={sentinelRef}
							aria-hidden
							style={{height: 1, width: '100%'}}
							data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.div--3"
						/>
						{isLoading && (
							<Spinner
								size="medium"
								data-flx="guild.guild-tabs.guild-audit-log-tab.legacy-guild-audit-log-tab.spinner--2"
							/>
						)}
					</div>
				)}
...
const GuildActivityLogTab: React.FC<{guildId: string}> = observer(({guildId}) =>
	GuildActivityLogPresentationRollout.enabled ? (
		<GuildAuditLogTab
			guildId={guildId}
			data-flx="guild.guild-tabs.guild-audit-log-tab.guild-activity-log-tab.guild-audit-log-tab"
		/>
	) : (
		<LegacyGuildAuditLogTab
			guildId={guildId}
			data-flx="guild.guild-tabs.guild-audit-log-tab.guild-activity-log-tab.legacy-guild-audit-log-tab"
		/>
	),
);

export default GuildActivityLogTab;
